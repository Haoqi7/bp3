...
class mail{
    ...
    {
        $server = $param['server'];  // 发送服务器
        $port = $param['port'];  // 服务器端口
        $user = $param['user'];  // 发送人
        $pass = $param['pass'];  // 客户端密钥
        $receiver = $param['receiver'];  // 客户端密钥
        $this->setServer($server,$user,$pass,$port,true);  // with ssl
        $this->setFrom($user);
        $this->setReceiver($receiver);  // 接收人
    }
     ...
     * 发送邮件
     * @access public
     * @param $subject  -- 邮件标题，文本
     * @param $body  -- 邮件内容，格式为html
     * @return boolean
     */
